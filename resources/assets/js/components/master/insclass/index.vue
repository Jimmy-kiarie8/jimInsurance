        ...
        <v-container fluid fill-height v-show="!loader">
            <v-layout justify-center align-center>
                <div> 
                    ...
                    <v-card-title>
                        Classes
                        <!-- <download-excel :data="AllClasses" :fields="json_fields">
                            Export
                            <img src="/storage/csv.png" style="width: 30px; height: 30px; cursor: pointer;">
                        </download-excel> -->
                            <v-btn slot="activator" color="primary" dark flat @click="openAdd">Add Class</v-btn>
                            <v-tooltip right>
                                <v-btn icon slot="activator" class="mx-0" @click="getClasses">
                    ...
                    <v-data-table :headers="headers" :items="AllClasses" class="elevation-1" :loading="loading" :search="search">
...
export default {
  ...
  methods: {
    ...
    openAdd() {
      eventBus.$emit("openaddClassEvent");
    },
    openEdit(item) {
      eventBus.$emit("openEditClassEvent", item);
    },
    showAlert() {
    ...
    deleteItem(item) {
      if (confirm("Are you sure you want to delete this item?")) {
        this.loading = true;
        const index = this.AllClasses.indexOf(item);
        axios
          .delete(`/insuraceclass/${item.id}`)
          .then(response => {
