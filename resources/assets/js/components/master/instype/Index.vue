        ...
        <v-container fluid fill-height v-show="!loader">
            <v-layout justify-center align-center>
                <div> 
                    ...
                    <v-card-title>
                        Types
                        <!-- <download-excel :data="AllTypes" :fields="json_fields">
                            Export
                            <img src="/storage/csv.png" style="width: 30px; height: 30px; cursor: pointer;">
                        </download-excel> -->
                            <v-btn slot="activator" color="primary" dark flat @click="openAdd">Add Type</v-btn>
                            <v-tooltip right>
                                <v-btn icon slot="activator" class="mx-0" @click="getTypes">
                    ...
                    <v-data-table :headers="headers" :items="AllTypes" class="elevation-1" :loading="loading" :search="search">
...
export default {
  ...
  methods: {
    ...
    openAdd() {
      // alert('test')
      eventBus.$emit("openaddTypeEvent");
      // this.dispAdd = true;
      // this.getCountry();
    },
    openEdit(item) {
      eventBus.$emit("openEditTypeEvent", item);
    },
    showAlert() {
    ...
    deleteItem(item) {
      if (confirm("Are you sure you want to delete this item?")) {
        this.loading = true;
        const index = this.AllTypes.indexOf(item);
        axios
          .delete(`/insuracetype/${item.id}`)
          .then(response => {
