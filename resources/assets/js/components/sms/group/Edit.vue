                    ...
                    <v-layout wrap v-show="!loader">
                        <v-form ref="form" @submit.prevent="submit">
                            <v-container grid-list-xl fluid>
                                <v-layout wrap>
                                    <v-flex xs12 sm6>
                                        <v-text-field v-model="form.contact_no" :rules="rules.name" color="blue darken-2" label="Contact Number" required></v-text-field>
                                        <!-- <small class="has-text-danger" v-if="errors.contact_no">{{ errors.contact_no[0] }}</small> -->
                                    </v-flex>
                                    <v-flex xs12 sm6>
...
export default {
    ...
    methods: {
        update() {
            this.loading = true
            axios.patch(`/contacts/${this.form.id}`, this.form).
            then((response) => {
                    ...
                    Object.assign(this.$parent.allContacts[this.$parent.editedIndex], this.$parent.editedItem)
